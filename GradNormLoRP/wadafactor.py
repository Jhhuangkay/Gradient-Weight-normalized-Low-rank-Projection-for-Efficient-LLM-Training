...
class WeightNormGaLoreAdafactor(Optimizer):
    """
    Adafactor optimizer with weight normalization and GaLore for low-rank gradient updates.
    """
    def __init__(
        self,
        params,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                grad_direction, grad_magnitude = state["weight_norm_galore"].compute_gradients(update)
                projected_grad = state["weight_norm_galore"].project_gradients(grad_direction, state["step"])

                # Update direction and magnitude
                state["weight_norm_galore"].direction_optimizer.zero_grad()
                state["weight_norm_galore"].direction.grad = projected_grad
                state["weight_norm_galore"].direction_optimizer.step()

                state["weight_norm_galore"].magnitude_optimizer.zero_grad()
                state["weight_norm_galore"].magnitude.grad = grad_magnitude
                state["weight_norm_galore"].magnitude_optimizer.step()
