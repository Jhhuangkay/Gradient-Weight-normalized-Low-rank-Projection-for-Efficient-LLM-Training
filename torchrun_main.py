...
from GradNormLoRP import GaLoreAdamW, GaLoreAdamW8bit, GaLoreAdafactor
...
def log_gpu_usage():
    ...
    logger.info(f"Total GPU Memory: {mem_info.total / (1024 ** 3):.2f} MB")
    logger.info(f"Used GPU Memory: {mem_info.used / (1024 ** 3):.2f} MB")
    logger.info(f"Free GPU Memory: {mem_info.free / (1024 ** 3):.2f} MB")
...
def main(args):
    ...
    if global_rank == 0:
        wandb.init(project="galore-c4_new")
    ...
    for batch_idx, batch in enumerate(dataloader):
        log_gpu_usage()  # Call to log GPU usage at each step
        global_step += 1
        local_step += 1
