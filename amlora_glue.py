...
from GradNormLoRP import GaLoreAdamW
...
def log_gpu_usage():
    """Logs the current GPU usage."""
    handle = pynvml.nvmlDeviceGetHandleByIndex(0)  # For GPU 0, adjust as necessary
    mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
    util_info = pynvml.nvmlDeviceGetUtilizationRates(handle)
    logger.info(f"Total GPU Memory: {mem_info.total / (1024 ** 3):.2f} GB")
    logger.info(f"Used GPU Memory: {mem_info.used / (1024 ** 3):.2f} GB")
    logger.info(f"Free GPU Memory: {mem_info.free / (1024 ** 3):.2f} GB")
    logger.info(f"GPU Utilization: {util_info.gpu}%")
    logger.info(f"PyTorch Allocated Memory: {torch.cuda.memory_allocated() / (1024 ** 3):.2f} GB")
    logger.info(f"PyTorch Reserved Memory: {torch.cuda.memory_reserved() / (1024 ** 3):.2f} GB")
...
def main():
    ...
    if args.task_name is not None:
        metric = evaluate.load("glue", args.task_name,experiment_id=str(unique_id))
    else:
        metric = evaluate.load("accuracy",experiment_id=str(unique_id))
